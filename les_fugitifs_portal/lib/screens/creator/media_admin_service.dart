...
class MediaAdminService {
  ...
  Future<_PickedMediaFile?> _pickFile() async {
    ...
    void completeSafely(_PickedMediaFile? value) {
      if (handled) return;
      handled = true;
      cancelTimer?.cancel();
      focusSub?.cancel();
    ...
    void completeErrorSafely(Object error) {
      if (handled) return;
      handled = true;
      cancelTimer?.cancel();
      focusSub?.cancel();
      ...
      try {
        ...
        reader.onLoadEnd.listen((_) {
          try {
            ...
            if (fileName.isEmpty) {
              completeErrorSafely(Exception('Nom de fichier invalide.'));
              return;
            }
    ...
    focusSub = html.window.onFocus.listen((_) {
      ...
      cancelTimer = Timer(const Duration(milliseconds: 300), () {
        final files = input.files;
        if (!handled && (files == null || files.isEmpty)) {
          completeSafely(null);
        }
      });
    });
  ...
  Uint8List? _bytesFromReaderResult(dynamic result) {
    ...
    try {
      ...
      if (maybeBuffer is ByteBuffer) {
        ...
        final int length = result.lengthInBytes is int
            ? result.lengthInBytes as int
            : maybeBuffer.lengthInBytes;
